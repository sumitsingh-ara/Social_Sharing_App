...
export const FriendsPending = () => {
    ...
    const getFriends = async () => {
      ...
      try {
        let data = await fetch(
          "http://localhost:7448/social/user/specificuser/allFriends/pending",
          {
            method: "GET",
            headers: {
  ...
  const acceptFriendRequest = async (senderId) => {
    ...
    try {
      let respo = await fetch(
        `http://localhost:7448/social/user/specificuser/respondAccept/${senderId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const declineFriendRequest = async (senderId) => {
    ...
    try {
      let respo = await fetch(
        `http://localhost:7448/social/user/specificuser/declineAccept/${senderId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const unFriend = async (friendId) => {
    ...
    try {
      let respo = await fetch(
        `http://localhost:7448/social/user/specificuser/unfriending/${friendId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className={styles.container}>
          ...
          >
            <div className={styles.imager}>
              <img
                ...
                src={
                  item.user.profilePic.image ? item.user.profilePic.image : url
                }
                alt=""
              />
            </div>
            <div className="mx-1">
              <h4>
                {item.user.name}{" "}
                <span className="text-muted" style={{ fontSize: "2.5vh" }}>
                  ({item.user.username})
                </span>
              </h4>
      ...
      <div className="text-center w-100">
        <h3>
          My Friends (
          {loading ? "Loading..." : data.friends ? data.friends.length : 0})
        </h3>
        {loading ? (
          <h1>Loading...</h1>
        ) : error ? (
